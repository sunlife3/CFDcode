    ...
    do j=0,Ny
        do i=0,Nx
            ...
            if(is_SF_xi(i,j) == 1)then
                ...
                E(i,j,1) = 0.4d0*E1(i,j,1) + 0.6d0*E2(i,j,1)
                E(i,j,2) = 0.4d0*E1(i,j,2) + 0.6d0*E2(i,j,2)
                E(i,j,3) = 0.4d0*E1(i,j,3) + 0.6d0*E2(i,j,3)
                E(i,j,4) = 0.4d0*E1(i,j,4) + 0.6d0*E2(i,j,4)
                ...
                !call SD_SLAU_Xflux(Q,E1,m,Nx,Ny,i,j,0.0d0)
                !call SD_SLAU_Xflux(Q,E2,m,Nx,Ny,i,j,4.0d0)                
                E(i,j,1) = 0.2d0*E1(i,j,1) + 0.8d0*E2(i,j,1)
                E(i,j,2) = 0.2d0*E1(i,j,2) + 0.8d0*E2(i,j,2)
                E(i,j,3) = 0.2d0*E1(i,j,3) + 0.8d0*E2(i,j,3)
                E(i,j,4) = 0.2d0*E1(i,j,4) + 0.8d0*E2(i,j,4)
            else
                !call AusmDV_XFlux(Q,E,m,Nx,Ny,i,j,4.0d0)
                !call HLL_XFlux(Q,E,m,Nx,Ny,i,j)
                !call AusmDV_XFlux(Q,E1,m,Nx,Ny,i,j,0.0d0)
                !call AusmDV_XFlux(Q,E,m,Nx,Ny,i,j,2.5d0)
                !call HLL_XFlux(Q,E1,m,Nx,Ny,i,j,0.0d0)
                !call HLL_XFlux(Q,E2,m,Nx,Ny,i,j,4.0d0)
                !call SD_SLAU_Xflux(Q,E1,m,Nx,Ny,i,j,2.5d0)
                call SD_SLAU_Xflux(Q,E,m,Nx,Ny,i,j,4.0d0)
            endif
            ...
            if(is_SF_eta(i,j) == 1)then
               ...
               F(i,j,1) = 0.4d0*F1(i,j,1) + 0.6d0*F2(i,j,1)
               F(i,j,2) = 0.4d0*F1(i,j,2) + 0.6d0*F2(i,j,2)
               F(i,j,3) = 0.4d0*F1(i,j,3) + 0.6d0*F2(i,j,3)
               F(i,j,4) = 0.4d0*F1(i,j,4) + 0.6d0*F2(i,j,4)
               !call HLL_YFlux(Q,F,n,Nx,Ny,i,j)
                ...
                !call SD_SLAU_YFlux(Q,F1,n,Nx,Ny,i,j,0.0d0)
                !call SD_SLAU_YFlux(Q,F2,n,Nx,Ny,i,j,4.0d0)    
                F(i,j,1) = 0.2d0*F1(i,j,1) + 0.8d0*F2(i,j,1)
                F(i,j,2) = 0.2d0*F1(i,j,2) + 0.8d0*F2(i,j,2)
                F(i,j,3) = 0.2d0*F1(i,j,3) + 0.8d0*F2(i,j,3)
                F(i,j,4) = 0.2d0*F1(i,j,4) + 0.8d0*F2(i,j,4)

            else
                !call AusmDV_YFlux(Q,F,n,Nx,Ny,i,j,4.0d0)
                !call HLL_YFlux(Q,F,n,Nx,Ny,i,j)
                !call AusmDV_YFlux(Q,F1,n,Nx,Ny,i,j,0.0d0)
                !call AusmDV_YFlux(Q,F,n,Nx,Ny,i,j,2.5d0)
                !call HLL_YFlux(Q,F1,n,Nx,Ny,i,j,0.0d0)
                !call HLL_YFlux(Q,F2,n,Nx,Ny,i,j,4.0d0)
                !call SD_SLAU_YFlux(Q,F1,n,Nx,Ny,i,j,0.0d0)
                call SD_SLAU_YFlux(Q,F,n,Nx,Ny,i,j,4.0d0)
            endif
